...
def _serialize_slot(slot: dict) -> dict:
    ...
    return {
        ...
        "peak_basket_close": slot["peak_basket_close"],
...
def save_state(portfolio_engine, ticker_buffers: dict,
    ...
    # Ticker buffers are no longer saved to reduce state file size,
    # as the warmup fetch retrieves full history from the broker on restart.
    ...
    with open(state_file, "w") as f:
        json.dump(state, f, cls=_Encoder)
...
def _deserialize_slot(raw: dict | None) -> dict | None:
    ...
    return {
        ...
        "peak_basket_close": raw.get("peak_basket_close"),
